...
def build_signup_view(app: "AurexFletApp") -> ft.View:
    page = app.page
    print("[aurex][signup] build_signup_view")
    ...
    def set_busy(is_busy: bool, message: str = "") -> None:
        print(f"[aurex][signup] set_busy is_busy={is_busy} message={message!r}")
    ...
    def handle_signup(_: ft.ControlEvent) -> None:
        print("[aurex][signup] Create Account clicked")
        ...
        def worker() -> None:
            try:
                print(f"[aurex][signup] worker start for username={username!r}")
                set_busy(True, "Creating your Aurex account...")
                print("[aurex][signup] connecting to server")
                app.connect_if_needed(discover_first=True)
                print("[aurex][signup] generating or loading local keys")
                ...
                print(f"[aurex][signup] public key ready len={len(public_key_b64)}")
                app.client.signup(username, password, email, public_key_b64=public_key_b64)
                print(f"[aurex][signup] signup request completed for {username!r}")
                ...
                print(f"[aurex][signup] worker error: {exc}")
                msg = str(exc)
                ...
                print(f"[aurex][signup] worker done for username={username!r}")
                set_busy(False)
